...
def calculate_lyrics_stats(artist: str, api: Api, silent: bool = False, output_pipe: TextIO = sys.stdout)\
        -> Tuple[List[int], int]:
    """ Returns the word count of all lyrics found, and total number of songs. """
    response = api.lookup_artist_interactive(artist, output_pipe=output_pipe)
    if not response:
        return [], 0
...
def run(args: List[str], file=sys.stdout, api: Api = Api()) -> None:
    ...
    word_counts, song_count = calculate_lyrics_stats(args.artist, api, output_pipe=file)
